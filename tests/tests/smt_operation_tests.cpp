...
BOOST_AUTO_TEST_CASE( smt_create_reset )
{
   try {
      ...
      smt_param_allow_vesting vesting;
      vesting.value = true;
      smt_param_allow_voting voting;
      voting.value = true;
      op4.setup_parameters.insert( vesting );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get< smt_token_object, by_symbol >( alice_prec4 ), [&]( smt_token_object& o )
         {
      ...
      tx.signatures.clear();
      sign( tx, alice_private_key );
      STEEM_REQUIRE_THROW( db->push_transaction( tx, 0 ) );
   }
   FC_LOG_AND_RETHROW()
}
